...
public:  
    ...
    {
             //RCLCPP_INFO(this->get_logger(), "called foot_print callback");
	     ::footprint = *p;
	     //RCLCPP_INFO(this->get_logger(), "ran foot_print callback");
    }

// accesses the odometry data
    void odom_callback(const nav_msgs::msg::Odometry::ConstPtr msg)
    {
             //RCLCPP_INFO(this->get_logger(), "called odom callback");
	     ::odom = *msg;
	     //RCLCPP_INFO(this->get_logger(), "ran odom callback");
    }
    void costmap_callback(const nav_msgs::msg::OccupancyGrid::ConstPtr occupancy_grid)
    {
            //RCLCPP_INFO(this->get_logger(), "called costmap callback");
		cost_count++;
    ...
    #pragma omp critical
		...
		for (long i = 0; i < ob1.size(); i++)
		{
	        ...
	        //RCLCPP_INFO(this->get_logger(), "ran costmap callback");
   }
