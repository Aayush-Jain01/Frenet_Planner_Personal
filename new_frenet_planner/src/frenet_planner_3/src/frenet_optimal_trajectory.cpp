...
void FrenetPath::adding_global_path(Spline2D csp)
{
	...
	int n = s.size();
	//printf("in adding_global_path - s size - %d\n",s.size());
	x.resize(n);
	//printf("in adding_global_path - x size - %d\n",n);
	y.resize(n);
	//printf("in adding_global_path - y size - %d\n",n);
	for (int i = 0; i < n; i++)
	{
		double ix, iy;
		...
		if (ix == NONE)
		{
			//printf("returning from adding_global_path\n");
			return;
		}
		double iyaw = csp.calc_yaw(s[i]);
		//printf("in adding_global_path - iyaw - %lf\n",iyaw);
		double fx = ix - d[i] * sin(iyaw);
		//printf("in adding_global_path - fx - %lf\n",fx);
		double fy = iy + d[i] * cos(iyaw);
		//printf("in adding_global_path - fy - %lf\n",fy);
		x[i] = (fx);
		y[i] = (fy);
		//printf("in adding_global_path - x,y - %lf,%lf", x[i],y[i]);
	}
	yaw.resize(n - 1);
	ds.resize(n - 1);
							  ...
							  double obst_r)
{
	...
	int tmpflag2 = 0;
	int tmpflag3 = 0;
	cout << "check_path_inside: here:" << __LINE__ << ":" << fplist.size() << endl;
#pragma omp parallel for collapse(1)
...
#pragma omp critical
		...
		++tmpflag3;
		if(fplist_final.size() == 0) {
				cout << "check_path: here:" << __LINE__ << endl;
		}
	}
	cout << "check_path: here:" << __LINE__ << ":" << fplist_final.size() << "tmpFlag1:" << tmpflag1 << "tmpflag2:" << tmpflag2 << "tmpflag3:" << tmpflag3 << endl;
	return fplist_final;
}
								   ...
								   double c_d_d, double c_d_dd, FrenetPath lp, double bot_yaw)
{
	...
	cout << "here:" << __LINE__ << fplist.size() << endl;
	double endTime1 = omp_get_wtime();
	...
	double cf;
	FrenetPath* bestpath_ptr = new FrenetPath();
	FrenetPath& bestpath = *bestpath_ptr;
	cout << "here:" << __LINE__ << fplist.size() << endl;
	for (auto &fp : fplist)
	{
		cf = fp.get_cf();
	...
	cout << "here:" << __LINE__ << bestpath.get_s() << endl;
	if (true)
	{
		plt::ion();
		plt::show();
	...
	// }
	cout << "BESTPATH" << bestpath.get_s() << endl;
	return bestpath;
}
