...
MatrixXd& Spline::calc_A(vecD h)  // get matrix A
{
  ...
  MatrixXd *Ap = new MatrixXd(nx, nx);
...
MatrixXd& Spline::calc_B(vecD h)  // get matrix B
{
  ...
  MatrixXd *Bp = new MatrixXd(nx, nx);
  MatrixXd& B = *Bp;
...
void Spline::init(vecD x_in, vecD y_in)  // calculates the coeffs for splines
{
  ...
  a = y;
  printf("before matrix calc A init\n");
  ...
  int Spline::search_index(double p)  // returns the index of the just greater element
{
  ...
  if(it == x.end())
  {
    ...
    return it-x.begin()-1;          //it-x.begin()
  }
}

  MatrixXd& A = calc_A(h);
  printf("before matrix calc B init\n");
  
  

  MatrixXd B = calc_B(h);
  printf("before matrix calc C init\n");
  
  MatrixXd C = A.inverse()*B;  // C = A^(-1)*B
  
  printf("after matrix calc init\n");
...
{
  printf("entering calc_spline_course\n");
  Spline2D *sp = new Spline2D(x, y);
  printf("mid  calc_spline_course\n");
  vecD s;
  double sRange = sp->get_s_last();
  double sInc = 0;
  while(1)
  {
    //printf("SInc = %lf, SRange = %lf", sInc, sRange); 
    if(sInc >= sRange)
    {
      break;
    }
  ...
  printf("exiting calc_spline_course\n");
